...
class Account {
public:
    Account(QString bank, int balance = 0)      // bank: 은행이름, balance: 초기 잔액
        : bank(bank), balance(balance) {}

    QString getBank()    const { return bank; }
    int     getBalance() const { return balance; }

    void deposit(int amount)  { balance += amount; }    // 입금시 잔액 증가 구현
    void withdraw(int amount) { balance -= amount; }    // 출금시 잔액 감소 구현
    ...
    QList<Transaction>& getHistory() { return history; }    // 거래 내역 목록 반환

private:
    QString            bank;        // 은행이름
    int                balance;     // 현재 잔액
    QList<Transaction> history;     // 거래내역 리스트
};
